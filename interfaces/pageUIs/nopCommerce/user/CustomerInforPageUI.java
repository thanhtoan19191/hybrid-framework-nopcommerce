package pageUIs.nopCommerce.user;

public class CustomerInforPageUI {
	public static final String CUSTOMER_INFOR_HEADER= "xpath=//div[@class='page-title']/h1[text()='My account - Customer info']";
	public static final String FEMALE_CHECKBOX= "xpath=//label[text()='Female']";
	public static final String EDIT_FIRST_NAME_TEXTBOX= "xpath=//input[@id='FirstName']";
	public static final String EDIT_LAST_NAME_TEXTBOX= "xpath=//input[@id='LastName']";
	public static final String DAY_DROPDOWN= "xpath=//select[@name='DateOfBirthDay']";
	public static final String MONTH_DROPDOWN= "xpath=//select[@name='DateOfBirthMonth']";
	public static final String YEAR_DROPDOWN= "xpath=//select[@name='DateOfBirthYear']";
	public static final String EDIT_EMAIL_TEXTBOX= "xpath=//input[@id='Email']";
	public static final String COMPANY_TEXTBOX= "xpath=//input[@id='Company']";
	public static final String SAVE_BUTTON= "xpath=//button[@id='save-info-button']";
	
}
